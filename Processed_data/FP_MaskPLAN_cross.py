...
class FloorPlan_RPlan_Dataset():
    ...
    def prepare_MaskPLAN_Data(self, num_layout, loops):
        ...
        np.savez_compressed('Processed_data/RPLAN_input_cross.npz',T=T_new,L=L_new,A=A_new,S=S_new,R=R_new)
        np.savez_compressed('Processed_data/RPLAN_output_cross.npz',T=T_ou,L=L_ou,A=A_ou,S=S_ou,R=R_ou)
        np.savez_compressed('Processed_data/RPLAN_mask_cross.npz',T=T_mask,L=L_mask,A=A_mask,S=S_mask,R=R_mask)
