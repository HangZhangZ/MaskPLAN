...
def MASKPLAN(embed_dim,latent_dim,num_heads,enc_layers,dec_layers):
    ...
    mask_sqe = Concatenate(axis=1)([type_embedding, loc_embedding, ada_embedding, area_embedding, room_embedding])
    ...
    loc_in = Input(shape=(sqe_len,code), dtype="int64", name="loc")
    ada_in = Input(shape=(sqe_len,sqe_len-2), dtype="int64", name="ada")
    area_in = Input(shape=(sqe_len), dtype="int64", name="area")
    room_in = Input(shape=(sqe_len,code), dtype="int64", name="room")

    type_embedding2 = PositionalEmbedding(sqe_len,11,embed_dim)(type_in)
    loc_embedding2 = PositionalEmbedding_IMG(sqe_len,embed_dim)(loc_in)
    ...
    room_embedding2 = PositionalEmbedding_IMG(sqe_len,embed_dim)(room_in)

    in_sqe = Concatenate(axis=1)([type_embedding2, loc_embedding2, ada_embedding2, area_embedding2, room_embedding2])
    ...
    x2 = mlp(x2,[latent_dim,embed_dim,code_dimen*code])
    x2 = Reshape((sqe_len, code, code_dimen))(x2)
    ...
    x4 = mlp(x4,[latent_dim,embed_dim,code_dimen*code])
    x4 = Reshape((sqe_len, code, code_dimen))(x4)
