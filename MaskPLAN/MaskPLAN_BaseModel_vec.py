...
class MaskPLAN_ModelS():
    def __init__(self,embed_dim,latent_dim,num_heads,enc_layers,dec_layers):
        ...
        bound_input = Input(shape=(code_dimen), dtype="float32")
        door_input = Input(shape=(2), dtype="float32")
        enc_layers2 = enc_layers#int(enc_layers/2)
        prior_weight = 2
        ...
        Generator_Encoder = GeneratorEncoder(embed_dim,transformer_units,num_heads,enc_layers2)
        encoder_outputs3 = Generator_Encoder(Dense(embed_dim, activation="LeakyReLU")(Concatenate()([flow_type,flow_loc,flow_ada])),encoded_inputs30)
        encoder_outputs3 = mlp(Concatenate()([Dense(prior_weight*embed_dim)(encoder_outputs3),encoded_inputs30]),transformer_units)
