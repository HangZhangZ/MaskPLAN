...
def MaskPLAN_GlobalEncoder(embed_dim,latent_dim,num_heads,enc_layers):
    ...
    down_units = [embed_dim, embed_dim / 5]
    ...
    bound_input = Input(shape=(code_dimen), dtype="int64", name="bound_input")
    ...
    return Model([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input], encoder_outputs, name='MAE_Encoder')
...
class MaskPLAN_ModelT():
    def __init__(self,embed_dim,latent_dim,num_heads,enc_layers,dec_layers):
        ...
        bound_input = Input(shape=(code_dimen), dtype="int64")
        ...
        get_mask_embedding0 = MaskPLAN_Encoder([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input])
        final_type = Decoder_Type([decoder_inputs0,get_mask_embedding0])  
        
        self.MaskPLAN_Type = Model([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input,decoder_inputs0], final_type, name='Mask_Type')

class MaskPLAN_ModelL():
    def __init__(self,embed_dim,latent_dim,num_heads,enc_layers,dec_layers):
        super().__init__()
        transformer_units = [latent_dim, embed_dim]
        down_units = [embed_dim, embed_dim / 2]
        type_mask = Input(shape=(sqe_len), dtype="int64")
        loc_mask = Input(shape=(sqe_len,code), dtype="int64")
        ada_mask = Input(shape=(sqe_len,sqe_len-2), dtype="int64")
        area_mask = Input(shape=(sqe_len), dtype="int64")
        ...
        Generator_Encoder = GeneratorEncoder(embed_dim,transformer_units,num_heads2,enc_layers2)
        encoder_outputs1 = Generator_Encoder(encoded_inputs10,flow_type)
        flow_type = mlp(flow_type,down_units)
        encoder_outputs1 = mlp(encoder_outputs1,down_units)
        encoder_outputs1 = Concatenate()([flow_type,encoder_outputs1])
        ...
        get_mask_embedding1 = MaskPLAN_Encoder([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input])
        Final_loc = Decoder_Loc([decoder_inputs1,encoded_inputs11,get_mask_embedding1])

        self.MaskPLAN_Loc = Model([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input,decoder_inputs1,encoded_inputs11], Final_loc, name="Mask_loc")
...
class MaskPLAN_ModelA():
    def __init__(self,embed_dim,latent_dim,num_heads,enc_layers,dec_layers):
        ...
        Generator_Encoder = GeneratorEncoder(embed_dim,transformer_units,num_heads2,enc_layers2)
        encoder_outputs2 = Generator_Encoder(encoded_inputs20,mlp(Concatenate()([flow_type,flow_loc]),transformer_units))
        flow_type = mlp(flow_type,down_units)
        flow_loc = mlp(flow_loc,down_units)
        encoder_outputs2 = mlp(Concatenate()([flow_loc,flow_type,encoder_outputs2]),transformer_units)
        ...
        get_mask_embedding2 = MaskPLAN_Encoder([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input])
        Final_ada = Decoder_Ada([decoder_inputs2,encoded_inputs21,encoded_inputs22,get_mask_embedding2])

        self.MaskPLAN_Ada = Model([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input,decoder_inputs2,encoded_inputs21,encoded_inputs22], Final_ada, name="Mask_ada")
...
class MaskPLAN_ModelS():
    def __init__(self,embed_dim,latent_dim,num_heads,enc_layers,dec_layers):
        ...
        Generator_Encoder = GeneratorEncoder(embed_dim,transformer_units,num_heads2,enc_layers2)
        encoder_outputs3 = Generator_Encoder(encoded_inputs30,mlp(Concatenate()([flow_type,flow_loc,flow_ada]),transformer_units))
        flow_type = mlp(flow_type,down_units)
        flow_loc = mlp(flow_loc,down_units)
        flow_ada = mlp(flow_ada,down_units)
        encoder_outputs3 = mlp(Concatenate()([flow_ada,flow_loc,flow_type,encoder_outputs3]),transformer_units)
        ...
        get_mask_embedding3 = MaskPLAN_Encoder([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input])
        Final_area = Decoder_Area([decoder_inputs3,encoded_inputs31,encoded_inputs32,encoded_inputs33,get_mask_embedding3])

        self.MaskPLAN_Area = Model([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input,decoder_inputs3,encoded_inputs31,encoded_inputs32,encoded_inputs33], Final_area, name="Mask_area")
...
class MaskPLAN_ModelR():
    def __init__(self,embed_dim,latent_dim,num_heads,enc_layers,dec_layers):
        ...
        flow_area = PositionalEmbedding(sqe_len,area_dimen,embed_dim)(encoded_inputs41)

        Generator_Encoder = GeneratorEncoder(embed_dim,transformer_units,num_heads2,enc_layers2)
        encoder_outputs4 = Generator_Encoder(mlp(Concatenate()([flow_type,flow_loc,flow_ada,flow_area]),transformer_units),encoded_inputs40)
        # flow_type = mlp(flow_type,down_units)
        # flow_loc = mlp(flow_loc,down_units)
        # flow_ada = mlp(flow_ada,down_units)
        # flow_area = mlp(flow_area,down_units)
        encoder_outputs4 = mlp(Concatenate()([encoder_outputs4,mlp(encoded_inputs40,down_units)]),transformer_units)
        ...
        get_mask_embedding4 = MaskPLAN_Encoder([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input])
        ...
        self.MaskPLAN_Room = Model([type_mask,loc_mask,ada_mask,area_mask,room_mask,bound_input,decoder_inputs4,encoded_inputs41,encoded_inputs42,encoded_inputs43,encoded_inputs44], Final_region, name="Mask_room")
