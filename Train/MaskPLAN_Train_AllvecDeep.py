...
def main(args):
    ...
    Mask_data = np.load('Processed_data/RPLAN_mask_vec.npz')
    Input_data = np.load('Processed_data/RPLAN_input_vec.npz')
    Output_data = np.load('Processed_data/RPLAN_output_vec.npz')
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint('MaskPLAN_Trained/All_%s_Deep_vec/All' % (args.model),"val_loss",0,True,True,"min","epoch",options=None,initial_value_threshold=None,)
