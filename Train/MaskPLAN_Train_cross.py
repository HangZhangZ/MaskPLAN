...
import numpy as np
import warnings
from MaskPLAN_BaseModel_cross import *
...
def main(args):
    ...
    reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='loss', factor=0.5,patience=1, min_lr=1e-7)
    
    LR_SCHEDULE = [(0, 1e-5),(60, 5e-6),(100, 2e-6),(150, 1e-6)]
    ...
    bound = np.load('Processed_data/RPLAN_bound.npy',allow_pickle=True).reshape(80788,64)
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint('MaskPLAN_Trained/%s_%s_cross/Model' % (args.part,args.model),"loss",0,True,True,"min","epoch",options=None,initial_value_threshold=None,)
    ...
    if args.part == 'Type':
        ...
        if args.resume == True:
            MaskPLAN_T.load_weights('MaskPLAN_Trained/Type_%s_cross/Model' % (args.model))
        MaskPLAN_T.compile(optimizer=optimizer,loss="sparse_categorical_crossentropy", metrics=["accuracy"])
        MaskPLAN_T.fit([Mask_T,Mask_L,Mask_A,Mask_S,Mask_R,bound,T_in],T_out,epochs=EPOCHS,batch_size=BATCH_SIZE,validation_split=args.val,shuffle=True,callbacks=callback)

    elif args.part == 'Location':

        MaskPLAN_L = MaskPLAN_ModelL(embed_dim,latent_dim,num_heads,enc_layers,dec_layers).MaskPLAN_Loc
        if args.resume == True:
            MaskPLAN_L.load_weights('MaskPLAN_Trained/%s_%s_cross' % (args.part,args.model))
        MaskPLAN_L.compile(optimizer=optimizer,loss="sparse_categorical_crossentropy", metrics=["accuracy"])
        MaskPLAN_L.fit([Mask_T,Mask_L,Mask_A,Mask_S,Mask_R,bound,L_in,T_out],L_out,epochs=EPOCHS,batch_size=BATCH_SIZE,validation_split=args.val,shuffle=True,callbacks=callback)

    elif args.part == 'Adjacency':

        MaskPLAN_A = MaskPLAN_ModelA(embed_dim,latent_dim,num_heads,enc_layers,dec_layers).MaskPLAN_Ada
        if args.resume == True:
            MaskPLAN_A.load_weights('MaskPLAN_Trained/Ada_%s_cross/Model' % (args.model))
        MaskPLAN_A.compile(optimizer=optimizer,loss="sparse_categorical_crossentropy", metrics=["accuracy"])
        MaskPLAN_A.fit([Mask_T,Mask_L,Mask_A,Mask_S,Mask_R,bound,A_in,T_out,L_out],A_out,epochs=EPOCHS,batch_size=BATCH_SIZE,validation_split=args.val,shuffle=True,callbacks=callback)

    elif args.part == 'Area':

        MaskPLAN_S = MaskPLAN_ModelS(embed_dim,latent_dim,num_heads,enc_layers,dec_layers).MaskPLAN_Area
        if args.resume == True:
            MaskPLAN_S.load_weights('MaskPLAN_Trained/Area_%s_cross/Model' % (args.model))
        MaskPLAN_S.compile(optimizer=optimizer,loss="sparse_categorical_crossentropy", metrics=["accuracy"])
        MaskPLAN_S.fit([Mask_T,Mask_L,Mask_A,Mask_S,Mask_R,bound,S_in,T_out,L_out,A_out],S_out,epochs=EPOCHS,batch_size=BATCH_SIZE,validation_split=args.val,shuffle=True,callbacks=callback)

    elif args.part == 'Region':

        MaskPLAN_R = MaskPLAN_ModelR(embed_dim,latent_dim,num_heads,enc_layers,dec_layers).MaskPLAN_Room
        os.makedirs('MaskPLAN_Trained/Room_%s_cross/Model' % (args.model),exist_ok=True)
        if args.resume == True:
            MaskPLAN_R.load_weights('MaskPLAN_Trained/Room_%s_cross/Model' % (args.model))
        MaskPLAN_R.compile(optimizer=optimizer,loss="sparse_categorical_crossentropy", metrics=["accuracy"])
        MaskPLAN_R.fit([Mask_T,Mask_L,Mask_A,Mask_S,Mask_R,bound,R_in,T_out,L_out,A_out,S_out],R_out,epochs=EPOCHS,batch_size=BATCH_SIZE,validation_split=args.val,shuffle=True,callbacks=callback)
