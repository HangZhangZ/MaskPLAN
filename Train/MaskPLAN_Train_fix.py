...
import numpy as np
import warnings
from MaskPLAN_BaseModel_fix import *
...
def main(args):
    ...
    if args.part == 'Type':
        ...
        os.makedirs('MaskPLAN_Trained/Type_%s_fix/Model_T' % (args.model),exist_ok=True)
        if args.resume == True:
            MaskPLAN_T.load_weights('MaskPLAN_Trained/Type_%s_fix/Model_T' % (args.model))
        checkpoint = tf.keras.callbacks.ModelCheckpoint('MaskPLAN_Trained/Type_%s_fix/Model_T' % (args.model),"loss",0,True,True,"min","epoch",options=None,initial_value_threshold=None,)
        ...
        os.makedirs('MaskPLAN_Trained/Loc_%s_fix/Model_L' % (args.model),exist_ok=True)
        if args.resume == True:
            MaskPLAN_L.load_weights('MaskPLAN_Trained/Loc_%s_fix/Model_L' % (args.model))
        checkpoint = tf.keras.callbacks.ModelCheckpoint('MaskPLAN_Trained/Loc_%s_fix/Model_L' % (args.model),"loss",0,True,True,"min","epoch",options=None,initial_value_threshold=None,)
        ...
        os.makedirs('MaskPLAN_Trained/Ada_%s_fix/Model_A' % (args.model),exist_ok=True)
        if args.resume == True:
            MaskPLAN_A.load_weights('MaskPLAN_Trained/Ada_%s_fix/Model_A' % (args.model))
        checkpoint = tf.keras.callbacks.ModelCheckpoint('MaskPLAN_Trained/Ada_%s_fix/Model_A' % (args.model),"loss",0,True,True,"min","epoch",options=None,initial_value_threshold=None,)
        ...
        os.makedirs('MaskPLAN_Trained/Area_%s_fix/Model_A' % (args.model),exist_ok=True)
        if args.resume == True:
            MaskPLAN_S.load_weights('MaskPLAN_Trained/Area_%s_fix/Model_A' % (args.model))
        checkpoint = tf.keras.callbacks.ModelCheckpoint('MaskPLAN_Trained/Area_%s_fix/Model_S' % (args.model),"loss",0,True,True,"min","epoch",options=None,initial_value_threshold=None,)
        ...
        os.makedirs('MaskPLAN_Trained/Room_%s_fix/Model_R' % (args.model),exist_ok=True)
        if args.resume == True:
            MaskPLAN_R.load_weights('MaskPLAN_Trained/Room_%s_fix/Model_R' % (args.model))
        checkpoint = tf.keras.callbacks.ModelCheckpoint('MaskPLAN_Trained/Room_%s_fix/Model_R' % (args.model),"loss",0,True,True,"min","epoch",options=None,initial_value_threshold=None,)
