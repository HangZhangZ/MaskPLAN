...
from MaskPLAN.MaskPLAN_BaseAll_vecDeep import *
...
def parse_args():
    ...
    parser.add_argument('--model', default='Base', type=str, help='Tiny, Base, Large')
...
def main(args):
    ...
    os.makedirs('Inference/%s_%s_%s/iteration/raw' % (args.model,args.deep,args.format),exist_ok=True)
    os.makedirs('Inference/%s_%s_%s/iteration/post' % (args.model,args.deep,args.format),exist_ok=True)
    ...
    if args.format == 'cross':
        ...
        # if args.deep == 'Deep': from MaskPLAN_BaseAll_crossDeep import *
        # elif args.deep == 'Single': from MaskPLAN_BaseAll_cross import *
        ...
        # if args.deep == 'Deep': from MaskPLAN_BaseAll_vecDeep import *
        # elif args.deep == 'Single': from MaskPLAN_BaseAll_vec import *    

    # elif args.format == 'hybrid': TODO

    #     dimen_L = 2
    #     dimen_R = code

    #     if args.deep == 'Deep': from MaskPLAN_BaseAll_hybridDeep import MASKPLAN
    #     elif args.deep == 'Single': from MaskPLAN_BaseAll_hybrid import MASKPLAN    
        ...
        def inference_interation(self,site_id):
            ...
            for k in range(int(list_len*5-1)):
                ...
                if 0<= k < list_len-1: # Type
                    ...
                    if (self.In_L[0,k-list_len*2+1] != np.zeros(2)).any().any(): continue # and (self.In_L[0,k-list_len*2+1] != np.zeros(code))
                    else: 
                    ...
                    if (self.In_R[0,k-list_len*4+1] != np.zeros(4)).any(): continue #  and (self.In_R[0,k-list_len*4+1] != np.zeros(code)).any()
                    else: 
            ...
            if args.format == 'vec':
                ...
                x_min = (corners[:,0]/loc_dimen)*(bound_domain[site_id,3] - bound_domain[site_id,1]) + bound_domain[site_id,1]
                y_min = (corners[:,1]/loc_dimen)*(bound_domain[site_id,2] - bound_domain[site_id,0]) + bound_domain[site_id,0]
                x_max = (corners[:,2]/loc_dimen)*(bound_domain[site_id,3] - bound_domain[site_id,1]) + bound_domain[site_id,1]
                y_max = (corners[:,3]/loc_dimen)*(bound_domain[site_id,2] - bound_domain[site_id,0]) + bound_domain[site_id,0]
